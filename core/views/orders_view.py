...
class UserBestellungenListView(ListView):
    ...
    def get_queryset(self):
        return Bestellung.objects.filter(
            person__benutzer=self.request.user).select_related(
                'schulungstermin__schulung').prefetch_related(
                    'schulungstermin__schulungsteilnehmer_set')

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if settings.IS_PRODUCTION and settings.DEBUG:
            self.stdout.write(
                style.ERROR("⚠️  WARNING: Debug is enabled in production!")
            )
        elif not settings.IS_PRODUCTION and settings.DEBUG:
            self.stdout.write(
                style.SUCCESS("✓ Debug is enabled for development/staging")
            )
        ...
        if "*" in settings.ALLOWED_HOSTS and settings.IS_PRODUCTION:
            self.stdout.write(
                style.ERROR("⚠️  WARNING: Wildcard in ALLOWED_HOSTS in production!")
            )
        elif "*" in settings.ALLOWED_HOSTS and not settings.IS_PRODUCTION:
            self.stdout.write(
                style.WARNING("⚠️  Wildcard in ALLOWED_HOSTS (OK for development)")
            )
        else:
        ...
        if hasattr(settings, "SECURE_HSTS_SECONDS"):
            ...
            self.stdout.write(
                f"Preload: {getattr(settings, 'SECURE_HSTS_PRELOAD', False)}"
            )
        ...
        if settings.SECRET_KEY:
            if (
                ...
                or "development" in settings.SECRET_KEY.lower()
            ):
                if settings.IS_PRODUCTION:
                    self.stdout.write(
                        style.ERROR(
                            "⚠️  WARNING: Using development secret key in production!"
                        )
                    )
                else:
                    self.stdout.write(
                        style.WARNING(
                            "⚠️  Using development secret key (OK for development)"
                        )
                    )
            else:
        ...
        if settings.IS_PRODUCTION:
            if (
                ...
                self.stdout.write(
                    style.SUCCESS("✓ Production security configuration looks good!")
                )
            else:
                self.stdout.write(
                    style.ERROR(
            ...
            self.stdout.write(
                style.SUCCESS(
                    f"✓ {settings.ENVIRONMENT.title()} environment configuration is appropriate"
                )
            )
